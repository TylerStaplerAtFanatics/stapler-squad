...
## Success Criteria
- [ ] `pkg/classifier` exists and is used by `stapler-squad`.
- [ ] `ssq-hooks` binary is built and functional.
- [ ] `gemini` call is intercepted and blocked if it violates rules.
- [ ] Real `gemini` call executes correctly if approved.
- [ ] Zero regressions in existing `stapler-squad` tests.
