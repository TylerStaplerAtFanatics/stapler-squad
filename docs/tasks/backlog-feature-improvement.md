...
| 4 | `autonomous_orchestration_service.go:248-276` | Role→status `switch` silently no-ops (log only) on an unrecognized role — a new pipeline stage added elsewhere would silently stop advancing items. |
| 5 | `backlog_service_triage.go:760-762` | `TransitionBacklogItemStatus` (idea→ready) after successful triage fails silently — item stuck in `idea` forever, zero operator signal. |
| 6 | `backlog_service_triage.go:748` | `UpdateItemSessionTriageResult` fails silently — triage output may not persist. |
| 7 | `backlog_service_triage.go:755-756` | `UpdateBacklogItem` (plan_artifacts_path) fails silently. |
| 8 | `backlog_service_triage.go:1016-1035` | Orphan-triage tombstone guard only runs on manual re-trigger — no standing detector for items stuck-after-successful-triage. |
| 9 | `ReconcileStuckItems` / `ArchiveBacklogItem` (per "Backlog History feature Broken" item's own planning notes) | Only `TransitionBacklogItemStatus` writes a `BacklogStatusEvent` audit row — these two paths mutate status directly via ent and skip event creation, extending the known notify-once-state gap. |
| — | Recurring pattern | Silent error-swallowing (`_ = s.storage.Update...(...)`) recurs across the service layer — same root cause as #2/#5/#6/#7, worth a single structural fix (e.g. a lint rule or wrapper) rather than N one-off fixes. |

No test coverage exists for the status-transition/notification-divergence path (#2) —
`autonomous_orchestration_service_test.go` only covers driver-registry lifecycle.
