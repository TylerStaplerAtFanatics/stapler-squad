...
func (_q *SessionQuery) QueryWorktree() *WorktreeQuery {
	query := (&WorktreeClient{config: _q.config}).Query()
...
func (_q *SessionQuery) QueryDiffStats() *DiffStatsQuery {
	query := (&DiffStatsClient{config: _q.config}).Query()
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		if err := _q.prepareQuery(ctx); err != nil {
			return nil, err
		}
...
// WithDiffStats tells the query-builder to eager-load the nodes that are connected to
// the "diff_stats" edge. The optional arguments are used to configure the query builder of the edge.
func (_q *SessionQuery) WithDiffStats(opts ...func(*DiffStatsQuery)) *SessionQuery {
	query := (&DiffStatsClient{config: _q.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
	_q.withDiffStats = query
	return _q
}
...
func (_q *SessionQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Session, error) {
	...
	if query := _q.withWorktree; query != nil {
		if err := _q.loadWorktree(ctx, query, nodes, nil,
			func(n *Session, e *Worktree) { n.Edges.Worktree = e }); err != nil {
			return nil, err
		}
	}
	if query := _q.withDiffStats; query != nil {
		if err := _q.loadDiffStats(ctx, query, nodes, nil,
			func(n *Session, e *DiffStats) { n.Edges.DiffStats = e }); err != nil {
			return nil, err
		}
	}
...
func (_q *SessionQuery) loadWorktree(ctx context.Context, query *WorktreeQuery, nodes []*Session, init func(*Session), assign func(*Session, *Worktree)) error {
	...
	query.withFKs = true
	query.Where(predicate.Worktree(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(session.WorktreeColumn), fks...))
	}))
	...
	for _, n := range neighbors {
		fk := n.session_worktree
		if fk == nil {
			return fmt.Errorf(`foreign-key "session_worktree" is nil for node %v`, n.ID)
		}
		node, ok := nodeids[*fk]
		if !ok {
			return fmt.Errorf(`unexpected referenced foreign-key "session_worktree" returned %v for node %v`, *fk, n.ID)
		}
		assign(node, n)
	}
	return nil
}
func (_q *SessionQuery) loadDiffStats(ctx context.Context, query *DiffStatsQuery, nodes []*Session, init func(*Session), assign func(*Session, *DiffStats)) error {
	fks := make([]driver.Value, 0, len(nodes))
	nodeids := make(map[int]*Session)
	for i := range nodes {
		fks = append(fks, nodes[i].ID)
	...
	query.withFKs = true
	query.Where(predicate.DiffStats(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(session.DiffStatsColumn), fks...))
	}))
	neighbors, err := query.All(ctx)
	if err != nil {
		return err
	...
	for _, n := range neighbors {
		fk := n.session_diff_stats
		if fk == nil {
			return fmt.Errorf(`foreign-key "session_diff_stats" is nil for node %v`, n.ID)
		}
		node, ok := nodeids[*fk]
		if !ok {
			return fmt.Errorf(`unexpected referenced foreign-key "session_diff_stats" returned %v for node %v`, *fk, n.ID)
		}
		assign(node, n)
	}
	return nil
...
func (_q *SessionQuery) loadClaudeSession(ctx context.Context, query *ClaudeSessionQuery, nodes []*Session, init func(*Session), assign func(*Session, *ClaudeSession)) error {
	fks := make([]driver.Value, 0, len(nodes))
	nodeids := make(map[int]*Session)
	for i := range nodes {
		fks = append(fks, nodes[i].ID)
	...
	query.withFKs = true
	query.Where(predicate.ClaudeSession(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(session.ClaudeSessionColumn), fks...))
	}))
	neighbors, err := query.All(ctx)
	if err != nil {
		return err
	...
	for _, n := range neighbors {
		fk := n.session_claude_session
		if fk == nil {
			return fmt.Errorf(`foreign-key "session_claude_session" is nil for node %v`, n.ID)
		}
		node, ok := nodeids[*fk]
		if !ok {
