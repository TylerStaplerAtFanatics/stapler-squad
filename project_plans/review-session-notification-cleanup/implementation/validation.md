...
|---|---|---|---|---|
| AC1: Hidden/`review`/`triage` sessions no longer generate generic TASK_COMPLETE/Idle/Stale notifications | `session/review_queue_determiner_test.go` | `TestDetermine_ReturnsSkip_When_InstanceHidden` | Unit | Happy path — `Hidden: true` inst with `IsControllerActive: true` + `ClaudeStatus: StatusSuccess` (would otherwise yield `ReasonTaskComplete`) returns `DetectionActionSkip` before the `ClaudeStatus` switch runs; second case covers the `IsControllerActive: false` + success-pattern-`content` branch |
| AC1 (cont.) | `session/review_queue_determiner_test.go` | Existing `Determine()` tests for non-Hidden instances (approval/error/idle detection) | Unit | Error/negative path — confirms the new early-return does not regress any existing non-Hidden detection branch; re-run as-is, no new assertions needed |
| AC1 (cont., belt-and-suspenders) | `session/startup_scanner_test.go` | `TestScan_SkipsHiddenInstance_AndNeverCallsStatusProvider` | Unit | Happy path — `Hidden: true`, `Started()`-true instance never reaches `queue.Add`, and a call-counting fake `StatusProvider`/`ContentProvider` records zero `GetStatus`/`GetContent` calls |
