...
- **Appetite / scope-cut check-in**: this plan ships the full CRUD management UI in Phase 3 (Epic 3.3: create/edit/enable/disable/delete via `/settings/pipeline-modes`), not a minimal/CLI-first cut, even though requirements.md's Appetite section pre-authorizes descoping it to "a minimal/admin-only form (or even a one-time seed script plus direct-DB editing)" if the Large (3–6 week) appetite is tight. This is a deliberate choice, recorded here rather than left implicit: Phase 1 and Phase 2 are independently shippable and already deliver the zero-regression seam + the full API surface a minimal form would also need, so Phase 3's UI is the only remaining appetite risk — if the budget is tight when Phase 3 starts, it can be descoped to a minimal form (or the pre-authorized seed-script/direct-DB-editing fallback) at that point without touching any Phase 1/2 work, using the same staged-shipping strategy above.
  **Adoption litmus test (pre-mortem P1, addresses the risk that Phase 3's UI investment goes unused because the target operator could just edit Go and redeploy instead):** before starting Phase 3, after Phase 2's CRUD RPCs ship, actually define a second mode's worth of content via a raw `grpcurl`/API call (bypassing the not-yet-built UI) and time it against editing `WriteSlashCommands` directly and running `make install-service`. Record the result in this paragraph. If the raw API call is not meaningfully lower-friction than a Go edit + redeploy for this specific operator, descope Phase 3 to the pre-authorized minimal/seed-script fallback above instead of building the full settings page — don't build Epic 3.3 on the assumption that a nicer UI alone justifies the phase's cost. This is a go/no-go gate on Phase 3's scope, not merely a nice-to-have data point.
