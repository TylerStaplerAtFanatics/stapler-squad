...
- Some repository-layer call sites now make two separate calls (one to `Notifier`, one to `ItemChangePublisher`) instead of one unified call — slightly more wiring surface per mutation site where both apply (in practice, only the stuck/bounce-adjacent call sites in `session/backlog_lifecycle.go` and `session/review_gate.go` overlap; the majority of the 6 hooked repository methods only ever call the new publisher).
