...
| 6 | Regression: systemd-style explicit port unchanged | `server/server_integration_test.go` | `Server_should_WriteUnchangedHookURL_When_StartedOnExplicitPort8543` | Integration | Error/regression path — explicit port path (no `PORT=0`), assert hook URL is still `http://localhost:8543/...`, proving Epic 1.3 didn't regress the default instance |
