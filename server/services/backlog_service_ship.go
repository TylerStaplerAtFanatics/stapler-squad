...
// hasUnshippedWorkSessionCode reports whether any work-role ItemSession for
// itemID has committed code (LastCommitSha != "") that has not been shipped
// via a PR yet (prURL == ""). Mirrors the guard the TransitionBacklogItemStatus
// RPC handler applies before allowing a review→done transition (see
// session.ErrPRRequired) — SubmitManualReview and TriggerReReview's headless
// PASS path both transition review→done via the storage layer directly
// (s.storage.TransitionBacklogItemStatus), which bypasses that RPC-handler-only
// guard entirely. Used by both to avoid silently marking an item "done" while
// its work session's commits were never pushed or turned into a PR — instead
// they leave the item in review, where this file's Ship PR action can recover it.
func hasUnshippedWorkSessionCode(ctx context.Context, storage *session.Storage, itemID, prURL string) bool {
	if prURL != "" {
		return false
	}
	itemSessions, err := storage.ListItemSessions(ctx, itemID)
	if err != nil {
		log.WarningLog.Printf("[hasUnshippedWorkSessionCode] ListItemSessions item=%s: %v", itemID, err)
		return false
	}
