...
func newRulesServiceWithAI(t *testing.T, aiClient AIClient) *RulesService {
	...
	return NewRulesService(rulesStore, NewConfigFileRulesStore(), analyticsStore, c, &DefaultRulePromptBuilder{}, aiClient)
}
...
func TestGenerateSuggestedRule_NilAIClient_ReturnsUnimplemented(t *testing.T) {
	...
	analyticsStore := NewAnalyticsStore(storage)
	c := classifier.NewRuleBasedClassifier()
	svc := NewRulesService(rulesStore, NewConfigFileRulesStore(), analyticsStore, c, nil, nil) // nil AI
...
func TestBuildPromptContext_IncludesRulesAndGaps(t *testing.T) {
	...
	c := classifier.NewRuleBasedClassifier()
	svc := NewRulesService(rulesStore, NewConfigFileRulesStore(), analyticsStore, c, &DefaultRulePromptBuilder{}, &mockAIClient{response: "[]"})
...
func TestAttachConflictInfo_SeedRuleAtHigherPriority_ShadowsSuggestion(t *testing.T) {
	...
	analyticsStore := NewAnalyticsStore(storage)
	c := classifier.NewRuleBasedClassifier()
	svc := NewRulesService(rulesStore, NewConfigFileRulesStore(), analyticsStore, c, nil, nil)
...
func newRulesServiceForCoverage(t *testing.T, specs []RuleSpec) *RulesService {
	...
	analyticsStore := NewAnalyticsStore(storage)
	c := classifier.NewRuleBasedClassifier()
	return NewRulesService(rulesStore, NewConfigFileRulesStore(), analyticsStore, c, nil, nil)
}
