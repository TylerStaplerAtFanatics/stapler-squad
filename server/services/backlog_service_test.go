...
// TestTriggerTriage_HungSessionTimedOut: a live triage session that started
// more than maxTriageDuration ago is tombstoned and re-trigger proceeds.
func TestTriggerTriage_HungSessionTimedOut(t *testing.T) {
	storage := createTestStorage(t)
	const liveUUID = "00000000-0000-0000-0000-000000000099"
	stopper := &mockSessionStopper{liveUUIDs: map[string]bool{liveUUID: true}}
	svc := NewBacklogService(storage, nil, nil, nil)
	svc.SetSessionStopper(stopper)

	createResp, err := svc.CreateBacklogItem(t.Context(), connect.NewRequest(&sessionv1.CreateBacklogItemRequest{
		Title:      "hung triage item",
		RepoPath:   t.TempDir(),
		SkipTriage: true,
	}))
	require.NoError(t, err)
	...
	// Insert a triage ItemSession started 3 hours ago (> maxTriageDuration).
	is, isErr := storage.CreateItemSession(t.Context(), session.ItemSessionData{
		ItemID:      itemID,
		SessionUUID: liveUUID,
		SessionRole: string(session.SessionRoleTriage),
	...
	require.NoError(t, isErr)
	hungStart := time.Now().Add(-3 * time.Hour)
	require.NoError(t, storage.UpdateItemSessionStarted(t.Context(), is.ID.String(), hungStart))

	// TriggerTriage must tombstone the hung session and proceed past the orphan guard.
	// The creator is nil, so it returns CodeUnimplemented once past the guard.
	// CodeAlreadyExists would mean the hung session was not tombstoned.
	_, trigErr := svc.TriggerTriage(t.Context(), connect.NewRequest(&sessionv1.TriggerTriageRequest{
		ItemId: itemID,
	}))
	require.Error(t, trigErr)
	var connErr *connect.Error
	require.ErrorAs(t, trigErr, &connErr)
	assert.NotEqual(t, connect.CodeAlreadyExists, connErr.Code(),
		"hung session should be tombstoned; got CodeAlreadyExists meaning guard blocked re-trigger")
}

// TestTriggerTriage_ConcurrentCallsSameItem: two simultaneous TriggerTriage
// calls on the same item must not both spawn sessions (TOCTOU prevention).
func TestTriggerTriage_ConcurrentCallsSameItem(t *testing.T) {
	storage := createTestStorage(t)
	svc := NewBacklogService(storage, nil, nil, nil)

	createResp, err := svc.CreateBacklogItem(t.Context(), connect.NewRequest(&sessionv1.CreateBacklogItemRequest{
		Title:      "concurrent triage item",
		RepoPath:   t.TempDir(),
		SkipTriage: true,
	}))
	require.NoError(t, err)
	...
	// Hold the in-progress lock manually to simulate a concurrent trigger.
	svc.triggerInProgress.Store(itemID, struct{}{})
	defer svc.triggerInProgress.Delete(itemID)
