...
func StartAnalyticsSubscriber(ctx context.Context, bus *events.EventBus, provider AnalyticsProvider) {
	if bus == nil || provider == nil {
		log.WarningLog.Printf("[analytics/subscriber] EventBus or provider is nil, not starting subscriber")
		return
	}
...
func recordFromEvent(ctx context.Context, provider AnalyticsProvider, event *events.Event) {
	...
	default:
		log.InfoLog.Printf("[analytics/subscriber] skipping untracked event type: %s", event.Type)
		return
	}

	if err := provider.Record(ctx, ae); err != nil {
		log.ErrorLog.Printf("[analytics/subscriber] Record error (event=%s): %v", ae.EventName, err)
	}
}
