...
// IT-4.3 — [BUG-1 fix] An unrelated EventSessionUpdated for an already-Stopped
// session (e.g. a rename, or RunOneShot republishing after discovering a
// github_pr_url) must NOT re-fire "Session Completed". Before real transition
// tracking was added, shouldNotify/buildStatusChangeNotification only checked
// the event's current status, so any later EventSessionUpdated for a session
// already at Stopped would look identical to a genuine Active→Stopped
// transition once the 2s dedup window (tag-based, not transition-aware) had
// elapsed. This test deliberately waits past that window to prove the fix
// relies on real transition detection, not the dedup window.
func TestSessionUpdatedRepublishAfterStopDoesNotRefireNotification(t *testing.T) {
	bus := events.NewEventBus(10)
	n := &mockNotifier{name: "test"}
	...
	inst := &session.Instance{ID: "sess-repub", Title: "My Session", Status: session.Stopped}
	bus.Publish(&events.Event{
		Type:    events.EventSessionUpdated,
		Session: inst,
	})
	...
	assert.Equal(t, 1, n.CallCount(), "initial Active→Stopped transition must trigger exactly one notification")

	// Wait past the dedup window so the old (dedup-only) protection would no
	// longer apply, proving the fix comes from transition tracking.
	time.Sleep(dedupWindow + 200*time.Millisecond)

	// Simulate an unrelated update to the already-stopped session — same
	// status, different field (e.g. a rename) — republishing EventSessionUpdated
	// without a genuine transition.
	renamed := &session.Instance{ID: "sess-repub", Title: "My Session (renamed)", Status: session.Stopped}
	bus.Publish(&events.Event{
		Type:          events.EventSessionUpdated,
		Session:       renamed,
	...
	// Give the subscriber time to process the second event, then confirm no
	// second notification was sent.
	time.Sleep(200 * time.Millisecond)
	assert.Equal(t, 1, n.CallCount(), "unrelated update to an already-Stopped session must not re-fire Session Completed")
}

// UT-4.4 — [BUG-2 fix] sweepExpiredSent removes lastSent entries whose dedup
// window has elapsed, and leaves fresh entries alone. This is the pure
// function driven by the delivery subscriber's periodic sweep ticker; testing
// it directly (rather than waiting for statusSweepInterval to fire for real)
// keeps the regression test fast and deterministic while still exercising the
// exact logic that bounds lastSent's memory growth over a long-running process.
func TestSweepExpiredSentRemovesOnlyStaleEntries(t *testing.T) {
	now := time.Now()
	lastSent := map[string]time.Time{
		"stale-tag": now.Add(-(dedupWindow + time.Second)),
		"fresh-tag": now,
	}

	sweepExpiredSent(lastSent, now, dedupWindow)

	_, staleStillPresent := lastSent["stale-tag"]
	_, freshStillPresent := lastSent["fresh-tag"]
	assert.False(t, staleStillPresent, "entries older than the dedup window must be swept")
